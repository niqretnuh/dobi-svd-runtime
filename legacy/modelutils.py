...
# added this because of incompatibility (Ali code requires my_utils and LowRankLinear, which I added)
def load_stage_fp16_checkpoint(model_dir, stage_pt="stage01_after_truncation_fp16.pt"):
    ...
    # LLaMA tokenizers often have no pad token; generation uses pad_token_id, so set it.
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token

    ckpt = torch.load(os.path.join(model_dir, stage_pt), map_location="cpu", weights_only=False)
    # Case A: ckpt is an actual HF model object
    if hasattr(ckpt, "state_dict") and not isinstance(ckpt, dict):
        state = ckpt.state_dict()
    # Case B: dict with state_dict
    elif isinstance(ckpt, dict) and "state_dict" in ckpt:
        state = ckpt["state_dict"]
    # Case C: raw state_dict dict
    ...
    print("Saved clean state_dict to:", clean_path)
    ...
    if isinstance(ckpt, dict) and "state_dict" in ckpt:
        ...
        raise ValueError(f"Unrecognized checkpoint format in {stage_pt}: {type(ckpt)}")
