...
def main(args):
    ...
    from modelutils import load_stage_fp16_checkpoint

    if not args.remapping:
        if os.path.exists(os.path.join(args.updated_model_path, "DobiSVD_Model.pt")):
            model, tokenizer = load_unremapping_model(args.updated_model_path)
        else:
            model, tokenizer = load_stage_fp16_checkpoint(args.updated_model_path, "stage01_after_truncation_fp16.pt")
